...
async function run(args: CliArgs) {
  ...
  if (args.bundlersContract) {
    console.warn(
      `Override bundlers contract address from initial state file, using: ${args.bundlersContract}`
    );
    initialState.bundlersContract = args.bundlersContract;
  }

  if (args.bundlerAddress) {
    console.warn(
      `Override bundler address from initial state file, using: ${args.bundlerAddress}`
    );
...
appArgs
  ...
  .option("-a, --bundler-address <address>", "Override bundler address")
  .option(
    "-c, --bundlers-contract <address>",
    "Override bundlers contract address"
  );
